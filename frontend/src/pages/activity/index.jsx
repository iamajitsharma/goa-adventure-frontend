...
const location=["Delhi","Mumbai","Kolkata","Chennai","Bengaluru","Mumbai","Kolkata","Chennai","Delhi","Mumbai","Kolkata"];
...
const onChange=()=>{
  ...
  return (
    <section className="">
      ...
      <div className=" flex flex-row  h-[60px] bg-white rounded-full justify-between items-center px-6 mx-12 mt-6 drop-shadow-lg">
        <div className="flex flex-row items-center ">
          <BsSearch />
          <input type="text" placeholder="Search..." className="border-none border-transparent focus:border-transparent focus:ring-0"/>
          <RxCross1  />
<div className="flex flex-row ml-10 mr-3"><span className="text-gray-400 text-sm">Sort By:</span><div>
<Dropdown options={options} onChange={onChange} value={defaultOption} placeholder="Select an option" />;</div></div>
        </div>
...
<div className="flex flex-row justify-between px-4 pt-3 pb-2">
  <button className="text-black  px-4 py-1 rounded-md font-semibold">Filter</button>
<button className="bg-orange-500 text-white px-4 py-1 rounded-md" >Reset</button>
</div>
<FilterLine />
...
{location.map((loc)=>(<FilterCheckBox label={loc} />))}

<div className="text-orange-500 font-semibold mx-6 my-2 cursor-pointer">Load More...</div>
...
{duration.map((loc)=>(<FilterCheckBox label={loc} />))}
